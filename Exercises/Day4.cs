...
namespace AdventCode.Exercises
{
    public static class Day4
    {
        ...
        {
            var passports = _passports.Split("\r\n\r\n")
                .Select(x => x.Replace("\r\n", " "))
                .ToList();
        ...
        private static List<Dictionary<string, string>> ParseData(List<string> listData)
        {
            var listDictionary = new List<Dictionary<string, string>>();

            foreach (var passport in listData)
            {
                if (!IsValidPassport(passport)) continue;
                ...
                passport.Split(" ")
                    ...
                    {
                        var fields = x.Split(":");
                        dictionary.Add(fields.First(), fields.ElementAt(1));
                    });
        ...
        {
            var hasByr = passport.Contains("byr", StringComparison.InvariantCultureIgnoreCase);
            var hasIyr = passport.Contains("iyr", StringComparison.InvariantCultureIgnoreCase);
            var hasEyr = passport.Contains("eyr", StringComparison.InvariantCultureIgnoreCase);
            var hasHgt = passport.Contains("hgt", StringComparison.InvariantCultureIgnoreCase);
            var hasHcl = passport.Contains("hcl", StringComparison.InvariantCultureIgnoreCase);
            var hasEcl = passport.Contains("ecl", StringComparison.InvariantCultureIgnoreCase);
            var hasPid = passport.Contains("pid", StringComparison.InvariantCultureIgnoreCase);
            var hasCid = passport.Contains("cid", StringComparison.InvariantCultureIgnoreCase);

            return (hasByr && hasIyr && hasEyr && hasHgt && hasHcl && hasEcl && hasPid);
        }

        private static bool ValidateLenght(string value, int lenght) => value.Length == lenght;
        private static bool ValidateRange(string value, int min, int max)
        {
            var entero = int.Parse(value);

            return entero >= min && entero <= max;
        }
        ...
        private static bool ValidateHeight(string value)
        {
            ...
            {
                var aux = value.Replace("cm", "");
                var entero = int.Parse(aux);

                return (entero >= 150 && entero <= 193);
            }
            else if (value.Contains("in"))
            {
                var aux = value.Replace("in", "");
                var entero = int.Parse(aux);

                return (entero >= 59 && entero <= 76);
            }
            else return false;
        }
        ...
        public static bool ValidatePassportNumber(string value)
        {
        ...
        public static int Exercise2()
        {
            ...
            var parsedData = ParseData(clearData);

            foreach (var dictionary in parsedData)
            {
                var isValidByr = ValidateLenght(dictionary["byr"], 4) && ValidateRange(dictionary["byr"], 1920, 2002);
                var isValidIyr = ValidateLenght(dictionary["iyr"], 4) && ValidateRange(dictionary["iyr"], 2010, 2020);
                var isValidEyr = ValidateLenght(dictionary["eyr"], 4) && ValidateRange(dictionary["eyr"], 2020, 2030);
                var isValidHgt = ValidateHeight(dictionary["hgt"]);
                var isValidHcl = ValidateHex(dictionary["hcl"]);
                var isValidEcl = ValidateHairColor(dictionary["ecl"]);
                var isValidPid = ValidatePassportNumber(dictionary["pid"]);

                if(isValidByr && isValidEyr && isValidHcl && isValidHgt && isValidIyr && isValidPid && isValidEcl)
                {
                    result++;
                }
            }
