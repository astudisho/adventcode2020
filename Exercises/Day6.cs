...
namespace AdventCode.Exercises
{
    ...
    {
        private static string[] _getGroups()
        {
            var groups = _data.Split("\r\n\r\n");

            return groups;
        }

        private static IEnumerable<string> _getAnswers(string[] data)
        {
            foreach (var answer in data)
            {
                var groups = answer.Replace("\r\n","");
                yield return groups;
            }
        }

        private static IEnumerable<IEnumerable<string>> _getAnswersByPeople(string[] data)
        {
            foreach (var answer in data)
            {
        ...
        public static int Exercise1()
        {
            ...
            var answers = _getAnswers(_getGroups()).ToList();

            foreach (var answer in answers)
            {
                var yesResponse = answer.Distinct().Count();
                result += yesResponse;
            }
        ...
        public static int Exercise2()
        {
            ...
            var answersByPeople = _getAnswersByPeople(groups).ToList();

            answersByPeople.ForEach(x => 
            {
                var answers = x.Aggregate("", (x, y) => x + y).Distinct();

                foreach (var letter in answers)
                {
                    if (x.All(x => x.Contains(letter)))
                    {
                        result++;
                    }
                }
