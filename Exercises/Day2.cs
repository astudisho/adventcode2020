...
namespace AdventCode.Exercises
{
    ...
    {
        private static List<PasswordPolicy> CleanRawInput()
        {
            var lines = _rawInput.Split('\n');

            List<PasswordPolicy> resultList = new List<PasswordPolicy>();

            foreach (var line in lines)
            {                
                var lineAux = line.Split(':');

                var firstSplit = lineAux[0].Split('-');
                var secondSplit = firstSplit.ElementAt(1).Split(' ');

                var minTimes = int.Parse(firstSplit.First());
                var maxTimes = int.Parse(secondSplit.First());
                var character = secondSplit.ElementAt(1);

                resultList.Add(new PasswordPolicy { Value = lineAux[1].Trim(), MinTimes = minTimes, MaxTimes = maxTimes, Character = character });
            }
        ...
        {
            var listOfPolicies = CleanRawInput();
            var resultCount = 0;

            foreach (var policy in listOfPolicies)
            {
                var count = policy.Value.Count(x => x == policy.Character.ToCharArray().First());

                if (count >= policy.MinTimes && count <= policy.MaxTimes) resultCount++;
            }
        }
        ...
        internal static void Exercise2()
        {
            ...
            foreach (var policy in listOfPolicies)
            {
                var character = policy.Character.ToCharArray().First();
                var xor = policy.Value[policy.MinTimes - 1] == character ^ policy.Value[policy.MaxTimes - 1] == character;
                if (xor) resultCount++;
            }
        }
        ...
        internal static string _rawInput =
